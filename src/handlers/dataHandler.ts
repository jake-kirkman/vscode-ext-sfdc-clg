...
export function validateRequiredSettings(pShowMessage: boolean = true) {
    ...
    if(missingConfigs.length != 0 && pShowMessage) {
        vscode.window.showWarningMessage(`You are missing required configurations. Navigate to Custom Label Extension settings and update the following: [${missingConfigs.join(', ')}]`);
    }
